...
int ReadJpegSections (FILE * infile, ReadMode_t ReadMode)
{
    ...
    a = fgetc(infile); //SLIVER_SOURCE

    if (a != 0xff || fgetc(infile) != M_SOI){ //SLIVER_SOURCE
        return FALSE;
    }
    ...
    for(;;){ //限制loop的bound为1
        ...
        prev = 0;
        for (a=0;;a++){
            marker = fgetc(infile); //SLIVER_SOURCE
        ...
        lh = fgetc(infile); //SLIVER_SOURCE
        ll = fgetc(infile); //SLIVER_SOURCE
        if (lh == EOF || ll == EOF){
            ErrFatal("Unexpected end of file");
        }
        ...
        got = fread(Data+2, 1, itemlen-2, infile); // Read the whole section. //SLIVER_SOURCE
        if (got != itemlen-2){
        ...
        switch(marker){
            ...
            case M_SOS:   // stop before hitting compressed data 
                ...
                if (ReadMode & READ_IMAGE){
                    ...
                    got = fread(Data, 1, size, infile); //SLIVER_SOURCE
                    if (got != size){
            ...
            case M_EXIF:
                ...
                if (ReadMode & READ_METADATA){
                            ...
                            //是否Data的长度受不受信任输入的影响 strlen(Data)
                    printf("%d\n",strlen(Data));//SLIVER_SINK
                    if (memcmp(Data+2, "Exif", 4) == 0){ //CVE-2020-26208 DATA的分配可能为0,len(data)写成变量
...
int ReplaceThumbnail(const char * ThumbFileName)
{
    ...
    if (ThumbnailFile){
        if (fread(ThumbnailPointer, 1, ThumbLen, ThumbnailFile) != ThumbLen){ //SLIVER_SOURCE
            goto noread;
        }
        fclose(ThumbnailFile);
    }
...
int RemoveSectionType(int SectionType)
{
    int a;
    int retval = FALSE;
    //是否SectionRead值受不确定性输入的影响
    //没有影响则两次执行的时候sink值相等。
    for (a=0;a<SectionsRead-1;a++){//SLIVER_SINK
        if (Sections[a].Type == SectionType){
            // Free up this section
            free (Sections[a].Data);
            // Move succeding sections back by one to close space in array.
            memmove(Sections+a, Sections+a+1, sizeof(Section_t) * (SectionsRead-a)); //CVE-2021-28278 
            SectionsRead -= 1;
            a -= 1;
            retval = TRUE;
        }
...
int RemoveUnknownSections(void)
{
    int a;
    int Modified = FALSE;
    //是否SectionsRead值受不确定性输入的硬性
    for (a=0;a<SectionsRead-1;){ //SLIVER_SINK
        switch(Sections[a].Type){
            ...
            default:
                ...
                free (Sections[a].Data);
                // Move succeding sections back by one to close space in array.
                memmove(Sections+a, Sections+a+1, sizeof(Section_t) * (SectionsRead-a)); //CVE-2021-28277 
                SectionsRead -= 1;
                Modified = TRUE;
        }
    }
